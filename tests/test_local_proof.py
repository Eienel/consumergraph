...
def test_real_sql_break_is_detected_and_repaired():
    ...
    assert result["consumergraph"]["verdict"] == "migration_required"
    assert len(result["consumergraph"]["affected_consumers"]) == 4
    assert "buyer_id AS customer_id" in result["consumergraph"]["generated_compatibility_sql"]
